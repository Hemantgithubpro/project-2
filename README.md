# EventSignal | Real-Time Event Notification Simulator

> An event-driven, queue-based notification system simulator with multi-channel delivery (Email, SMS, Push), worker auto-scaling, dead-letter queue (DLQ) inspection, and live device mockups.
...
* **Multi-Channel Notification Engine**:
  * **Email**: SendGrid SMTP simulator with HTML preview client.
  * **SMS**: Twilio API simulator with chat bubble preview.
  * **Push Notifications**: Firebase Cloud Messaging (FCM) simulator with lock-screen push banners.
...
* **Resiliency, Retries & Dead Letter Queue (DLQ)**:
...
- **Icons**: Lucide React
- **Event Engine**: In-Browser EventBus, Queue Engine, Worker Pool State Machine
...
1. **Clone the repository**:
   ```bash
   git clone https://github.com/YOUR_USERNAME/eventsignal.git
   cd eventsignal
   ```
...
├── src/
│   ├── components/
│   │   ├── Header.jsx                # Navigation & engine controls
│   │   ├── DispatchConsole.jsx       # Event producer & batch generator
│   │   ├── QueueArchitectureView.jsx # Pipeline diagram & worker pool status
│   │   ├── LiveDeliveryTracker.jsx   # Telemetry log table & DLQ inspector
│   │   ├── DeviceSimulators.jsx      # Live Push, SMS & Email receivers
│   │   └── MetricsDashboard.jsx      # System SLA & telemetry analytics
│   ├── engine/
│   │   ├── NotificationQueueEngine.js# Core event router & queue engine
